document.addEventListener('DOMContentLoaded', () => {
  ...
  async function tryFetch(path, opts={}) {
    try { const r = await fetch(path, opts); return r.ok ? r : null; }
    catch { return null; }
  }
  ...
  function ensureViewer() {
    ...
    if (!wrap) {
      ...
      const section = document.querySelector('section[aria-labelledby="consTitulo"] .body');
  ...
  function openInViewer(href, title) {
    ...
    if (!wrap) { alert('No se pudo crear el visor'); return; }
 ...
 async function loadConsentimientos() {
  ...
  listEl.innerHTML = '<div class="item"><span class="pill">Cargando…</span> Consultando /api/consentimientos…</div>';
  ...
  if (!resp) {
    listEl.innerHTML = '<div class="item">No pude leer <b>/api/consentimientos</b>. Verifica el controlador Nest.</div>';
    return;
  }

  const archivos = await resp.json();
  if (!Array.isArray(archivos) || archivos.length === 0) {
    listEl.innerHTML = '<div class="item">No hay archivos HTML en <b>/public/Consentimientos</b>.</div>';
    return;
  }
  ...
  archivos
    ...
    .forEach((file) => {
      ...
      item.querySelector('button').addEventListener('click', () => {
        ...
        openNew.href = href;
        frame.src = href;                // ← carga en el iframe
        wrap.hidden = false;
        wrap.scrollIntoView({ behavior: 'smooth', block: 'start' });
      });
